      ...
      <main className="max-w-275 mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_272px] gap-6 items-start">
        <div className="flex flex-col gap-5">
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            <div className="p-5">
              <div className="w-full aspect-21/6 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center text-gray-400 cursor-pointer hover:bg-gray-100">
                <FiImage size={24} />
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 py-2 border-b border-gray-50 flex items-center gap-1 bg-gray-50/50">
              <button
                ...
                className={`p-2 rounded-lg ${editor?.isActive('bold') ? 'bg-white text-indigo-600' : 'text-gray-400'}`}
              >
                <FiBold size={16} />
              </button>
              <button
                onClick={() => editor?.chain().focus().toggleItalic().run()}
                className={`p-2 rounded-lg ${editor?.isActive('italic') ? 'bg-white text-indigo-600' : 'text-gray-400'}`}
              >
                <FiItalic size={16} />
              </button>
              <button
                onClick={() => editor?.chain().focus().toggleBulletList().run()}
                className={`p-2 rounded-lg ${editor?.isActive('bulletList') ? 'bg-white text-indigo-600' : 'text-gray-400'}`}
              >
                <FiList size={16} />
              </button>
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="aspect-4/3 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center text-gray-300"
                >
                  <FiImage size={18} />
                </div>
              ))}
        ...
        <aside className="flex flex-col gap-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
            ...
            <select
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, isPublished: e.target.value === 'true' }))
              }
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 text-[12px] space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{wordCount} คำ</span>
            </div>
          </div>
