    ...
    return (
        <div className="bg-white border border-gray-100/80 rounded-3xl shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col h-full">
            <div className="px-7 py-6 border-b border-gray-50 flex justify-between items-center bg-white/50 backdrop-blur-xl">
                ...
                <Link href="blogs" className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-xl transition-colors">
                    ดูทั้งหมด
                </Link>
            </div>
                        ...
                        {blogs.map((blog) => (
                            <tr key={blog.id} className="hover:bg-gray-50/80 transition-colors group">
                                <td className="px-7 py-5 font-bold text-gray-900 w-1/2">
                                    <div className="line-clamp-1 group-hover:text-indigo-600 transition-colors">{blog.title}</div>
                                </td>
                      ...
                      ${blog.isPublished
                                ...
                                <td className="px-7 py-5 text-right">
                                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Link href={`blogs/edit/${blog.id}`} className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors">
                                            <FiEdit2 size={16} />
                                        </Link>
                                    </div>
                                </td>
