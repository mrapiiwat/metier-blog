    ...
    return (
        <div className="bg-white border border-gray-100/80 rounded-3xl shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col h-full">
            <div className="px-7 py-6 border-b border-gray-50 flex justify-between items-center bg-white/50 backdrop-blur-xl">
                ...
                <Link href="comments" className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-xl transition-colors shrink-0">
                    ดูทั้งหมด
                </Link>
            </div>
            ...
            <div className="divide-y divide-gray-50 overflow-y-auto flex-1 max-h-125">
                {comments.length === 0 ? (
                    ...
                    comments.map((comment) => (
                        <div key={comment.id} className="p-6 hover:bg-gray-50/50 transition-colors flex flex-col gap-4">
                            ...
                            <div className="flex items-center justify-between mt-1">
                                ...
                                <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-bold border ${aiStatusColors[comment.aiAnalysis.status]}`}>
                                    {comment.aiAnalysis.status === 'pending' ? <FiClock size={12} /> : <BsStars size={12} />}
