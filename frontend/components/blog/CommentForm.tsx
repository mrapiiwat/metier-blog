...
const CommentForm: React.FC<CommentFormProps> = ({ blogId }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50/50 border border-gray-100 p-6 rounded-2xl">
            <p className="text-xs text-gray-400">
                <span className='text-red-500'>*</span> หมายเหตุ: ทุกความคิดเห็นจะถูกตรวจสอบโดยผู้ดูแลระบบก่อนแสดงผลบนเว็บไซต์
            </p>

            <div className="space-y-1.5">
                <label htmlFor="author" className="text-xs font-bold text-gray-700">ชื่อผู้ส่ง</label>
                <input
                    type="text"
                    id="author"
                    value={author}
            ...
            <div className="space-y-1.5">
                <label htmlFor="comment" className="text-xs font-bold text-gray-700">ข้อความความคิดเห็น</label>
                <textarea
                    id="comment"
                    rows={4}
                    value={commentText}
            ...
            {error && <p className="text-xs font-medium text-red-500 bg-red-50 p-3 rounded-lg">{error}</p>}
            {success && (
